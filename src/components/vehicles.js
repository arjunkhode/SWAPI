...
import { setVehiclesPage } from '../actions/index';
...
firstPeoplePage(){
	...
	this.props.setVehiclesPage(1);
 	this.props.fetchPeople('vehicles',1);
}

lastPeoplePage(){
	let n = Math.floor(this.props.items.count/10);
	this.props.setPage(n);
	this.props.setVehiclesPage(n);
 	this.props.fetchPeople('vehicles',n);
}
...
function mapStateToProps(state) {
	return { items: state.items.all,
		...
		vehiclesPage: state.vehiclesPage.page };
}

export default connect(mapStateToProps, { fetchPeople, swapLanguage, setPage, setVehiclesPage })(Vehicles);
